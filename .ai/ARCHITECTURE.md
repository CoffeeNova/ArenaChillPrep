...
- **createItem**: cast (`waitingForCast`, 10 s timeout) → STOP/SUCCEEDED → **advances on cast-end unless the NEXT step is also a `createItem` step whose RESOLVED cast equals the current step's** (a repeated conjure — `WorkflowItemSteps:nextRepeatsConjure`). When the next step repeats the conjure, the old delta-based wait is kept (`ACP_ITEMS_CHANGED` fast path + 0.25 s `waitForItem` poll) so the next step's goal-met skip sees the item; otherwise the item wait is skipped entirely (the client grants the item ~1–1.5 s after STOP, so this removes that latency between non-repeated conjure steps).
...
| createItem cast done, item not in bags yet | advance on cast-end unless the next step repeats the same conjure (`nextRepeatsConjure`) — then keep the `ACP_ITEMS_CHANGED` + 0.25 s `waitForItem` poll (10 s safety timeout) so the next step's skip sees the item |
...
22. **createItem completes on cast-end; item wait stays only before a repeated conjure (2026-08-26).** `WorkflowCastController:onCastComplete` advances immediately on `UNIT_SPELLCAST_STOP`/`SUCCEEDED` instead of waiting ~1–1.5 s for the item to land in bags — UNLESS the NEXT step is also a `createItem` step whose RESOLVED cast (`resolveCastInfo`) equals the current step's (`WorkflowItemSteps:nextRepeatsConjure`), in which case the old delta wait (`waitForItem` + `ACP_ITEMS_CHANGED` fast path + 10 s safety) is kept so the next step's goal-met skip sees the item. This removes the inter-step latency for non-repeating conjures (e.g. Conjure Water → Conjure Food) while preserving the 2× Healthstone / 2× Water-Food-pair semantics. **equipItem grace (2026-08-26):** because a preceding `createItem` now advances without its item in bags, `equipItem` no longer treats an absent item as "already equipped"; it polls up to `WORKFLOW_EQUIP_GRACE` (3 s, tick-count cap — deterministic under `SyncTimers`) for the in-flight item, arms the equip when it appears, and only advances as already-equipped if the grace elapses. The equip completion poll (`WorkflowItemPoll`) and the grace poll (`WorkflowEquipGrace`) are both cancelled in `WorkflowEngine:cancelTimers`.

23. **Run-scoped `showAllSpellRanks` override (2026-08-31).** Hidden spell ranks fizzle on the secure cast path (CONTEXT gotcha #26). The engine keeps the CVar at "1" for the whole workflow run — from `start()`/`startTest()` (only when `WorkflowItemSteps:needsSpellRanks` finds a createItem step below its family's max rank) until DONE/`reset()`/`stopTest()`/`PLAYER_LOGOUT` (`restoreSpellRanks`, exact previous value, nil → "0" — the CVar persists in config-cache.wtf, so the logout restore is required). Pause/resume do NOT restore — a resumed run may cast the same hidden-rank step again. The pattern is proven by M6 ActionBook and WeakAuras LibDispel on the same client. `resolveCastInfo`/goal-met/`nextRepeatsConjure` are untouched: a stored rank-5 step still creates the Major stone.
