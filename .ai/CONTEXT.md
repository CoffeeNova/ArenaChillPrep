...
- **Comments are rare (user rule 2026-08-25).** Code must speak for itself. Do NOT write: historical narration ("was X, now Y", "fixed on <date>", "live-verified", regression stories — history lives in `.ai/` docs/git), restatements of what the code says, or multi-line explanations. Comments are allowed ONLY for a genuinely non-obvious WHY (e.g. a client quirk the code works around) — one short line. Keep `---@param`/`---@return` annotations (LSP contracts) and one short line per module/file stating its responsibility. When explaining a workaround is necessary, prefer a single terse line; the full story goes to `.ai/` (memories/ARCHITECTURE).
...
23. **`knowsSpell` must check the RESOLVED cast rank, not the stored ID (fixed 2026-08-25).** On 20506 a trained higher rank REPLACES the lower one in the spellbook, and `IsPlayerSpell(name)` does NOT match by base name — so the old stored-ID check skipped every step whose stored rank was not the player's current rank: Demon Armor (706), Create Healthstone ranks 1-5 (only 27230 worked), Create Spellstone ranks 1-3 (only 28172 worked) — live-reported 2026-08-25. `knowsSpell` now resolves via `resolveCastInfo` (family → highest KNOWN rank) and checks the resolved ID; a step whose whole family is unknown (e.g. a removed catalog spell) still resolves to the stored ID and is skipped. The static catalog stores MAX-rank-only entries for ranked buffs (Demon Armor = **27260**, user-verified; the old 706 was never castable at 70); `SettingsMigrator:migrateStepSpellIDs` rewrites saved 706 → 27260 (same pattern as 6307 → 19028). **Create Spellstone ranks 1-3 (2362/28171/28173) were REMOVED from `stoneRanks` (2026-08-25, user decision — no arena use; live: rank 1 cast verbatim, ranks 2-3 upgraded to Master anyway)** — only rank 4 (28172/22646) remains in the Add Step menu; saved lower-rank createItem steps are migrated to 28172/22646.
24. **Pre-existing buff ≠ cast completion (fixed 2026-08-25).** With "Skip completed steps" OFF, `waitForInstantEffect` used to treat an ALREADY-present buff (e.g. the 10-minute Unending Breath from the previous arena) as "the cast landed" and auto-advanced the step on the first poll tick with NO key press — the option effectively never turned skipping off for buff steps. The wait now snapshots the aura's `expirationTime` at step start (`WorkflowEngine:getBuffExpiration`): a buff step completes on mere presence only when the buff was ABSENT at start; an already-buffed target requires evidence of a NEW cast — the expiration changing (a refresh) or a detected press (SENT).
25. **createItem advances on cast-end; equipItem waits for an in-flight item (2026-08-26).** `WorkflowCastController:onCastComplete` no longer waits ~1–1.5 s for the conjured item to hit the bags — it advances immediately on `UNIT_SPELLCAST_STOP`/`SUCCEEDED` UNLESS the NEXT step is also a `createItem` step whose RESOLVED cast (`resolveCastInfo`) equals the current step's (`WorkflowItemSteps:nextRepeatsConjure`): a repeated conjure (2× Healthstone, 2× Water/Food pairs) keeps the old delta wait (`waitForItem` + `ACP_ITEMS_CHANGED` + 10 s safety) so the next step's goal-met skip sees the item. Because a preceding `createItem` now advances without its item, `equipItem` no longer treats an absent item as "already equipped" — it polls up to `WORKFLOW_EQUIP_GRACE` (3 s; tick-count cap, deterministic under `SyncTimers`) for the in-flight item, arms the equip when it appears, and only advances as already-equipped if the grace elapses. The grace poll (`WorkflowEquipGrace`) is cancelled in `WorkflowEngine:cancelTimers` alongside `WorkflowItemPoll`.
26. **Spellbook "Show all spell ranks" toggle gates hidden-rank CASTING (fixed 2026-08-31).** With `showAllSpellRanks = "0"` a trained-but-hidden lower rank (e.g. Create Healthstone 11730) silently FIZZLES on the secure cast path: no SENT, no FAILED, no error — the engine sat in `waitingForKey` forever on the rank-5 step (live-reported: workflows 1-2 stalled at "Create Healthstone (rank 5)"; the log ends at the item-arrival line). `IsPlayerSpell` still reports hidden ranks, so `resolveCastInfo`/goal-met worked — only the CAST was refused. Fix: a run-scoped override of the `showAllSpellRanks` CVar ("1" during the run, restore the exact previous value on DONE/reset/stopTest/PLAYER_LOGOUT) — enabled only when the definition contains a createItem step below its family's max rank (`WorkflowItemSteps:needsSpellRanks` + `enableSpellRanks`/`restoreSpellRanks`, called from `WorkflowEngine:start`/`reset`/DONE/`_init` logout handler). Pattern proven by working addons on the same client: M6 `Libs/ActionBook/Categories.lua:82-101` and WeakAuras `Libs/LibDispel/LibDispel.lua:158/220` ("fix a problem where spells dont show as existing because they are 'hidden'"). CVar name: `showAllSpellRanks` (case-insensitive; M6's lowercase form). The stored rank is still honored — a rank-5 step still creates the Major stone (the user explicitly rejected resolving it up to Master).
