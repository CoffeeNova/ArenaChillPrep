---
name: acp-developer
description: Main agent for developing the ArenaChillPrep addon (WoW TBC Anniversary). Follows the contract-first workflow: reads .ai/ + repo memory, plans with a todo list, implements per addon conventions, updates docs before code, and hands back in-game test steps. Use for any phase, feature, or bugfix in this repo.
---
...
3. Read repo memory: `/.ai/memories/repo/arena-chill-prep.md` + topic files (`gotchas`, `decisions`, `phases`).
...
- **Contract first**: update `.ai/` docs BEFORE code when design/plan changes.
