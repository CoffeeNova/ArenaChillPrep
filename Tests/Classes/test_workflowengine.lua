...
-- Regression: a pet step after a cast-time step is armed to be pressed during the cast.
...
-- Regression: a completed workflow must NOT go a second round — after DONE the key is a no-op.
...
-- Regression: on TBC 2.5.5 the stone ranks COEXIST — a castable rank-5 step
-- must not be satisfied by a leftover Master stone.
...
-- Regression (2026-08-25): a stored lower stone rank (11730, unlearned at 70)
function testKnowsSpellResolvesUnlearnedStoneRankViaFamily()
    -- Arrange: player knows only rank 6 of Create Healthstone.
    _G.__stub.knownSpells = { [27230] = true };
    resetSpellbook();

    -- Act + Assert: a stored rank-5 step resolves to rank 6 -> known.
    lu.assertTrue(Engine:knowsSpell({ type = "createItem", spellID = 11730, itemID = 19012 }));

    -- No rank of the family known -> still skipped as unknown.
    _G.__stub.knownSpells = {};
    lu.assertFalse(Engine:knowsSpell({ type = "createItem", spellID = 11730, itemID = 19012 }));

    -- Cleanup
    _G.__stub.knownSpells = {};
end

-- Demon Armor ships as the TBC max rank (27260); a legacy 706 step resolves to it.
function testKnowsSpellResolvesDemonArmorMaxRank()
    _G.__stub.knownSpells = { [27260] = true };
    _G.__stub.spellInfo = { [706] = "Demon Armor", [27260] = "Demon Armor" };
    resetSpellbook();
    Spellbook:addStaticFallback();

    -- Act + Assert: the stored max rank is known directly.
    lu.assertTrue(Engine:knowsSpell({ type = "cast", spellID = 27260, target = "player" }));

    -- A legacy rank-1 step resolves through the family name to the max rank.
    lu.assertTrue(Engine:knowsSpell({ type = "cast", spellID = 706, target = "player" }));
...
-- Regression (2026-08-25): Create Soulstone (693) and Ritual of Summoning
-- (698) were REMOVED from the catalog — a saved step with either is skipped
-- as unknown (no catalog entry, no known spell) instead of stalling.
function testKnowsSpellSkipsRemovedCatalogSpells()
    _G.__stub.knownSpells = {};
    _G.__stub.spellInfo = {};
    resetSpellbook();

    lu.assertFalse(Engine:knowsSpell({ type = "createItem", spellID = 693, itemID = 22103 }));
    lu.assertFalse(Engine:knowsSpell({ type = "cast", spellID = 698, target = "player" }));

    _G.__stub.knownSpells = {};
    _G.__stub.spellInfo = {};
end

-- Regression (2026-08-25): Create Spellstone ranks 1-3 (2362/28171/28173)
function testKnowsSpellResolvesRemovedSpellstoneRanksToMax()
    _G.__stub.knownSpells = { [28172] = true };
    _G.__stub.spellInfo = { [28171] = "Create Spellstone" };
    resetSpellbook();

    lu.assertTrue(Engine:knowsSpell({ type = "createItem", spellID = 28171, itemID = 13602 }));

    local castSpellID, castItemID = Engine:resolveCastInfo({ spellID = 28171, itemID = 13602 });
    lu.assertEquals(castSpellID, 28172);
    lu.assertEquals(castItemID, 22646);

    _G.__stub.knownSpells = {};
    _G.__stub.spellInfo = {};
end

-- getBuffExpiration: the named helpful aura's expirationTime (used by the
-- buff-refresh detection in waitForInstantEffect).
function testGetBuffExpiration()
    _G.__stub.auraByIndex = { name = "Unending Breath", expirationTime = 123 };
    lu.assertEquals(Engine:getBuffExpiration("player", "Unending Breath"), 123);

    -- A different aura name -> nil.
    _G.__stub.auraByIndex = { name = "Fel Armor", expirationTime = 456 };
    lu.assertNil(Engine:getBuffExpiration("player", "Unending Breath"));
...
-- Regression: "Skip completed steps by default" OFF — an already-buffed target
-- must still require a REAL re-cast (refresh), not auto-complete.
function testWaitForInstantEffectRequiresRefreshWhenAlreadyBuffed()
    -- Arrange
    ...
    local savedKnown = _G.__stub.knownSpells;
    _G.__stub.spellInfo = { [5697] = "Unending Breath" };
    _G.__stub.auraByIndex = { name = "Unending Breath", expirationTime = 100 };
    resetSpellbook();
    Spellbook:addStaticFallback();
    Engine:_init();
    applyFreshSlotBindings(key);
    ...
    local savedDef = ACP.Settings.WorkflowData.definitions[1];
    ACP.Settings.WorkflowData.definitions[1] = {
        enabled = true, name = "ub-refresh", steps = {
            { type = "cast", spellID = 5697, target = "player" },
            { type = "pet", spellID = 27269, spellName = "Fire Shield", target = "player" },
        },
    };

    -- Act 1: arm the buff step (skip is OFF and the buff is present -> the
    -- step must be armed for a re-cast, not skipped).
    Engine:executeCurrentStep();
    lu.assertEquals(Engine.waitingForKey, true, "step armed for a re-cast");

    -- Act 2: poll tick with the buff still as it was at step start (no press,
    -- no refresh) -> the step must NOT complete.
    H.SyncTimers.Handles["WorkflowGCD"].cb();
    lu.assertEquals(Engine.stepIndex, 1, "still on the buff step");
    lu.assertEquals(Engine.state, "RUNNING");

    -- Act 3: the user's re-cast refreshed the aura (new expiration) -> the
    -- step completes.
    _G.__stub.auraByIndex = { name = "Unending Breath", expirationTime = 800 };
    H.SyncTimers.Handles["WorkflowGCD"].cb();
    lu.assertEquals(Engine.stepIndex, 2, "the refresh completed the step");
...
-- Companion: a NOT-buffed target completes on the buff landing (the presence
-- signal), even though SENT may not fire for instant spells.
function testWaitForInstantEffectCompletesWhenBuffLands()
    -- Arrange
    local origTimers = ACP.Utils.Timers;
    ACP.Utils.Timers = H.SyncTimers;
    H.SyncTimers.Handles = {};
    ...
    local savedDef = ACP.Settings.WorkflowData.definitions[1];
    ACP.Settings.WorkflowData.definitions[1] = {
        enabled = true, name = "ub-land", steps = {
            { type = "cast", spellID = 5697, target = "player" },
            { type = "pet", spellID = 27269, spellName = "Fire Shield", target = "player" },
        },
    };

    -- Act 1: arm the buff step (buff absent at step start).
    Engine:executeCurrentStep();
    lu.assertEquals(Engine.waitingForKey, true);

    -- Act 2: the cast lands (buff appears) -> the step completes.
    _G.__stub.auraByIndex = { name = "Unending Breath", expirationTime = 700 };
    H.SyncTimers.Handles["WorkflowGCD"].cb();
    lu.assertEquals(Engine.stepIndex, 2, "the landing completed the step");
...
-- Regression: healthstone ranks 1-5 are historical item-ID pairs; the client
-- conjures ONE variant per rank.
...
-- Regression: party-targeted casts landed on the PLAYER (unit token unresolved).
...
-- Regression: the client silently swallows a pet ability pressed early in the cast.
...
-- warlock) always counts; "pet gone" counts only during the player's cast.
...
-- Regression: a step configured for a party member must not cast on the wrong unit.
...
-- Regression: arming a pet step during a SUMMON (pet not out yet) interrupted the cast.
...
-- Regression: a workflow slot can be bound to TWO keys — both must override.
